...
1. **SQL** — `rag/recommend.py::matchup_stats()`: fact + formation + staging JOIN (`3869253` 실측)
2. **pgvector** — `rag/embedder.py::search_similar()` (recommend.py `retrieve_context`): `embedding_documents` top-k  
   임베딩: 로컬 bge-small(384). 대상: WC 경기 리포트 + 상대 전술 패턴 + 한국/브라질 선수 프로파일
3. **LLM** — `rag/recommend.py::generate()` = **Google Gemini `gemini-2.5-flash`(프리티어)**. `sql_context + vector_context + user_input` → 포메이션·11인·**출처 명시 근거**. CLI `football-recommend`.
...
[✅] Day 5: 하이브리드 라인업 추천 — SQL→pgvector→Gemini, `football-recommend` (§1)
```
...
| Day 5 ✅ | 하이브리드 라인업 추천: "한국 vs 브라질 최적 라인업" — SQL→pgvector→**Gemini(프리티어)** 3단계, `football-recommend`, `src/football/rag/recommend.py` |
...
| 2026-06-23 | 임베딩=로컬 bge-small(384), LLM=**Gemini gemini-2.5-flash**(Claude에서 변경) — 둘 다 비용0(로컬/프리티어) |
